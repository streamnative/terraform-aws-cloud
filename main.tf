...
locals {
  ...
  snc_node_config = {
    for i, v in var.private_subnet_ids : "snc-node-pool${i}" =>
    merge(
      local.node_pool_defaults,
      {
        subnets = [var.private_subnet_ids[i]],
        name    = "snc-node-pool${i}"
      },
      var.node_pool_overrides
    )
  }
  snc_extra_node_config = (
    length(var.extra_node_pool_instance_types) == 0 ? {} :
    {
      for i, v in var.private_subnet_ids : "snc-extra-node-pool${i}" =>
      merge(
        local.node_pool_defaults,
        {
          subnets          = [var.private_subnet_ids[i]],
          name             = "snc-extra-node-pool${i}",
          instance_types   = var.extra_node_pool_instance_types,
          min_capacity     = var.extra_node_pool_min_size,
          max_capacity     = var.extra_node_pool_max_size,
          desired_capacity = var.extra_node_pool_desired_size
        },
        var.extra_node_pool_overrides
      )
    }
  )
