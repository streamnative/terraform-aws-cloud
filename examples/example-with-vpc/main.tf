...
module "sn_cluster" {
  ...
  pulsar_namespace        = "pulsar-demo"                 # The namespace you anticipate using for Pulsar
  pulsar_namespace_create = true                          # Have the module create the Pulsar namespace. This is recommended. 
  region                  = var.region
  s3_bucket_name_override = format("%s-offload", local.cluster_name) # Use this to override the default S3 bucket name for tiered storage offloading
  vault_prefix_override   = local.cluster_name                       # Use this to override the default Vault prefix for Vault resources
