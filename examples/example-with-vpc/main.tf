...
provider "kubernetes" {
  ...
  insecure               = false
  config_path            = "/path/to/my-sn-platform-cluster-config" # add this path to the provider configs above
}
...
module "sn_platform_cluster" {
  ...
  kubeconfig_output_path = "/path/to/my-sn-platform-cluster-config"
  ...
  # Adding a dependency on the VPC module allows for a clean destroy
  depends_on = [
    module.vpc
  ]
}
